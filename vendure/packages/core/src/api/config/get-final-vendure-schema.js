...
var plugin_metadata_1 = require("../../plugin/plugin-metadata");
var generate_active_order_types_1 = require("./generate-active-order-types");
var generate_auth_types_1 = require("./generate-auth-types");
var generate_error_code_enum_1 = require("./generate-error-code-enum");
var generate_list_options_1 = require("./generate-list-options");
var generate_permissions_1 = require("./generate-permissions");
var graphql_custom_fields_1 = require("./graphql-custom-fields");
function getFinalVendureSchema(options) {
    return __awaiter(this, void 0, void 0, function () {
        var config, typePaths, typesLoader, apiType, normalizedPaths, typeDefs, schema;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    config = options.config, typePaths = options.typePaths, typesLoader = options.typesLoader, apiType = options.apiType;
                    normalizedPaths = typePaths.map(function (p) { return p.split(path_1.default.sep).join('/'); });
                    return [4 /*yield*/, typesLoader.mergeTypesByPaths(normalizedPaths)];
                case 1:
                    typeDefs = _a.sent();
                    schema = (0, index_1.buildSchema)(typeDefs);
                    schema = buildSchemaFromVendureConfig(schema, config, apiType);
                    if (options.output === 'sdl') {
                        return [2 /*return*/, (0, index_1.printSchema)(schema)];
                    }
                    else {
...
function buildSchemaFromVendureConfig(schema, config, apiType) {
    var authStrategies = apiType === 'shop'
...
function extendSchemaWithPluginApiExtensions(schema, plugins, apiType) {
    (0, plugin_metadata_1.getPluginAPIExtensions)(plugins, apiType)
        .map(function (e) { return (typeof e.schema === 'function' ? e.schema(schema) : e.schema); })
        .filter(shared_utils_1.notNullOrUndefined)
        .forEach(function (documentNode) { return (schema = (0, index_1.extendSchema)(schema, documentNode)); });
    return schema;
}
